...
public class FuelSim {
  ...
  /**
   * Adds a fuel onto the field. If at max capacity, removes the oldest fuel first.
   *
   * @param pos Position to spawn at
   * @param vel Initial velocity vector
   */
  public void spawnFuel(Translation3d pos, Translation3d vel) {
    if (fuels.size() >= MAX_FUEL_COUNT) {
      fuels.remove(0); // Remove oldest fuel to maintain limit
    }
